...
% by convention and for the sake of standization, eigenvectors are usually
% scaled to have a distance equal to 1. To do this with the eigenvector in
% this example, first find the distance of the vector and divide it by that
% amount:
...
data = randn(100,1);
data(:,2) = data+randn(100,1);
corr(data)

[m,n] = size(data);  % m is # of observations & n is # of dimensions
...
cov_mat = cov(data); % gives square matrix with each dimension's variance on the diagonals
...
[pc,score,latent]=pca(data); % or princomp() gives same output
