function [imgRgb, overlayMask, slLabel, numColsRows] = mrAnatOverlayMontage_kh_ROI(overlayImg, xform, anatImg, anatXform, cmap, overlayClipRng, acpcSlices, fname, plane, alpha, labelFlag, upsamp, numAcross, clusterThresh, autoCropBorder)
...
% % %         imgRgb(overlayMask) = (1-alpha).*imgRgb(overlayMask) + alpha.*overlay(overlayMask);
        ...
        imgRgb(fixInd) = 1;
        % take out redundant 3rd dimension for returned mask
        overlayMask = overlayMask(:,:,1);
    end
end
figure;
