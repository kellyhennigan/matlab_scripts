function [imgRgb, overlayMask, slLabel, numColsRows] = mrAnatOverlayMontage_kh(overlayImg, xform, anatImg, anatXform, cmap, overlayClipRng, acpcSlices, fname, plane, alpha, labelFlag, upsamp, numAcross, clusterThresh, autoCropBorder)
...
%
% kjh, hacked it for displaying multiple overlays with different (user defined) colormaps
...
%% edit to use number of colors given in the colormap argument
...
% % %         imgRgb(overlayMask) = (1-alpha).*imgRgb(overlayMask) + alpha.*overlay(overlayMask);
        nBins = length(cmap);
        inds = round(overlay*(nBins-1)+1);
        inds(isnan(inds)) = 1;
        overlay = reshape(cmap(inds,:),[size(overlay) 3]);
        overlay(~overlayMask|isnan(overlay)) = 0;
        imgRgb(overlayMask) = (1-alpha).*imgRgb(overlayMask) + alpha.*overlay(overlayMask);
        % make any values > 1 equal to [1, 1, 1]
        fixInd = find(imgRgb>1);
        imgRgb(fixInd) = 1;
        % take out redundant 3rd dimension for returned mask & make it a
        % numeric instead of logical mask
        numericMask = zeros(size(overlayMask));
        numericMask(overlayMask) = 1;
        overlayMask = numericMask;
        end
end
% figure;
