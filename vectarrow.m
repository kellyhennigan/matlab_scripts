function h=vectarrow(p0,p1,rgbVal,lineW)
...
% edited by Kelly to take in rgbVal and linewidth params
%% 

% if no rgb values are given, plot it in black
if notDefined('rgbVal')
    rgbVal = [0 0 0];
end

% if no line width is given, use 5
if notDefined('lineW')
    lineW = 2;
end
...
%%
  ...
  h=        plot3([x0;x1],[y0;y1],[z0;z1],'color',rgbVal,'LineWidth',lineW);   % Draw a line between p0 and p1
...
%           hold on
              ...
              plot3(hu(:),hv(:),hw(:),'color',rgbVal,'LineWidth',lineW)  % Plot arrow head
...
%           hold off
  ...
  h=            plot([x0;x1],[y0;y1],'color',rgbVal,'LineWidth',lineW);   % Draw a line between p0 and p1
...
%           hold on
              ...
              plot(hu(:),hv(:),'color',rgbVal,'LineWidth',lineW)  % Plot arrow head
